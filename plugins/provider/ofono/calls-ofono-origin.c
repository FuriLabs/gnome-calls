                       ...
                       CallsOfonoOrigin       *self)
{
  ...
  ok = gdbo_ip_multimedia_system_call_get_properties_finish (ims, &properties, res, &error);
  if (!ok) {
         ...
         CallsOfonoOrigin     *self)
{
  ...
  if (self->ims)
    gdbo_ip_multimedia_system_call_get_properties (self->ims,
                                                   NULL,
                         ...
                         struct CallsVoiceCallProxyNewData *data)
{
  ...
  call = calls_ofono_call_new (voice_call, data->properties, self->voice, self->ims_registered);
            ...
            CallsOfonoOrigin *self)
{
  g_autoptr (GError) error = NULL;
  self->ims_registered = FALSE;
  self->ims = gdbo_ip_multimedia_system_proxy_new_finish (res, &error);
  if (!self->ims) {
    g_warning ("Error creating oFono"
